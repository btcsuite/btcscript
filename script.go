...
func SignatureScript(tx *btcwire.MsgTx, idx int, subscript []byte, hashType byte, privKey *btcec.PrivateKey, compress bool) ([]byte, error) {
...
func signTxOutputCustomReader(reader io.Reader, tx *btcwire.MsgTx, idx int,
	subScript []byte, hashType byte, key *btcec.PrivateKey) ([]byte, error) {
	...
	return append(signature.Serialize(), hashType), nil
}

func p2pkSignatureScript(tx *btcwire.MsgTx, idx int, subScript []byte, hashType byte, privKey *btcec.PrivateKey) ([]byte, error) {
	sig, err := signTxOutput(tx, idx, subScript, hashType, privKey)
	if err != nil {
		return nil, err
	}
